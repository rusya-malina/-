...
from states import TRAINING_EMPLOYEE, TRAINING_UPLOAD
...
async def training_employee_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text(f"✅ Выбран сотрудник: **{allowed['name']}**", parse_mode="Markdown")
    await context.bot.send_message(
        chat_id=query.message.chat_id,
        text=(
            f"📚 Отправьте Excel-файл для сотрудника **{allowed['name']}**.\n"
...
async def process_training_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not recipient_id.isdigit():
        await update.message.reply_text("ℹ️ Выбор сотрудника устарел. Нажмите «Загрузить обучение» ещё раз.")
        return ConversationHandler.END
    ...
    if not is_training_group(group) or not any(item["user_id"] == recipient_id for item in candidates):
        ...
        context.user_data.pop("training_recipient_id", None)
        context.user_data.pop("training_recipient_name", None)
        return ConversationHandler.END
    ...
    try:
        ...
        await context.bot.send_message(
            chat_id=int(recipient_id),
            text="📚 Вам отправлено учебное задание от координатора.",
        )
        await update.message.reply_text(
            f"✅ Excel-файл обучения отправлен сотруднику **{recipient_name}**.",
        ...
        await update.message.reply_text(
            f"❌ Не удалось отправить файл сотруднику **{recipient_name}**. Попробуйте ещё раз.",
